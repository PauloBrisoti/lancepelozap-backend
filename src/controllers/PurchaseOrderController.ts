...
export class PurchaseOrderController {
  listCreditCards = asyncHandler(async (req: Request, res: Response) => {
  ...
  create = asyncHandler(async (req: Request, res: Response) => {
    ...
    // Frete de compra compõe a dívida (parcelas do Contas a Pagar),
    // contraparte do frete capitalizado no custo do estoque.
    const valorTotalLiquido = valorTotalBruto - desconto + freteValor;
  ...
  update = asyncHandler(async (req: Request, res: Response) => {
    ...
    // Mantém o frete já registrado no pedido compondo o total da dívida
    const valorTotalLiquido = valorTotalBruto - desconto + Number(existing.valorFrete || 0);
  ...
  receive = asyncHandler(async (req: Request, res: Response) => {
    ...
    // Frete de compra agrega ao custo do estoque (contrato 2.2) — rateio proporcional ao valor do pedido
    // Desconto do pedido é rateado proporcionalmente no custo unitário (custo = valor efetivamente pago)
    // O rateio usa o total do PEDIDO (não o lote recebido) para que recebimentos parciais não
    // capitalizem o frete mais de uma vez.
    const freteValor = Number(existing.valorFrete) || 0;
    const brutoPedido = Number(existing.valorTotalBruto) || 0;
    const descontoPedido = Number(existing.valorDesconto) || 0;
    const fatorDesconto = brutoPedido > 0 ? (brutoPedido - descontoPedido) / brutoPedido : 1;
    let totalPedidoValor = 0;
    for (const orderItem of existing.items) {
      totalPedidoValor += Number(orderItem.quantidade) * Number(orderItem.precoUnitario) * fatorDesconto;
    }
    ...
    for (const received of receivedItems) {
      ...
      if (isCustomerOrder) {
        ...
        const precoUnitarioEfetivo = Number(orderItem.precoUnitario) * fatorDesconto;
        const freteShare = totalPedidoValor > 0
          ? (qtdReceber * precoUnitarioEfetivo / totalPedidoValor) * freteValor
          : 0;
        const custoAnteriorTotal = saldoAnterior * Number(product.precoCusto);
        const custoEntradaTotal = qtdReceber * precoUnitarioEfetivo + freteShare;
        const novoCustoMedio = saldoPosterior > 0
          ? (custoAnteriorTotal + custoEntradaTotal) / saldoPosterior
          : precoUnitarioEfetivo + (qtdReceber > 0 ? freteShare / qtdReceber : 0);
  ...
  cardInvoices = asyncHandler(async (req: Request, res: Response) => {
    const storeId = getStoreId(req);
  ...
  payCardInvoice = asyncHandler(async (req: Request, res: Response) => {
    const storeId = getStoreId(req);
