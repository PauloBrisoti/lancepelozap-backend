...
export class DreController {
  static getDre = asyncHandler(async (req: Request, res: Response) => {
    ...
    // 6. Lucro Bruto Operacional (impostos não entram na estrutura da DRE conforme contrato)
    ...
    const transacoesSaida = await prisma.financialTransaction.groupBy({
      ...
      where: {
        ...
        categoria: { notIn: ['DEVOLUCAO', 'PRO_LABORE', 'RETIRADA_LUCRO', 'CANCELAMENTO', 'PAGAMENTO_FORNECEDOR', 'COMPRA_ESTOQUE'] } // Devoluções, lucros, cancelamentos e compras de mercadoria não são despesas operacionais da DRE
      },
      _sum: { valor: true }
    });
  ...
  static exportDre = asyncHandler(async (req: Request, res: Response) => {
    const storeId = getStoreId(req);
