...
export class DashboardV2Controller {
  static getTenantDashboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  }, "carregar dashboard V2");

  static getPjMetrics = asyncHandler(async (req: Request, res: Response) => {
    const storeId = getStoreId(req);
    ...
    const [aggEntrada, aggSaida, wallets] = await Promise.all([
      prisma.financialTransaction.aggregate({
    ...
    const capitalLivre = saldoAtual + aReceberFiado - parcelasFornecedoresMes;
