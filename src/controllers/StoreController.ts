...
export class StoreController {
  ...
  async updateCardBehavior(req: Request, res: Response) {
    try {
      const clientId = req.user?.clientId as string;
      const storeId = req.params.id as string;
      ...
      console.error('Erro ao atualizar comportamento de cartão:', error);
      return fail(res, 'Erro ao atualizar comportamento de cartão', 500);
    }
  }

  async updateFiscalMonthStart(req: Request, res: Response) {
    try {
      const storeId = req.params.id as string;
      ...
      console.error('Erro ao atualizar dia de início do mês:', error);
      return fail(res, 'Erro interno', 500);
    }
  }

  async getFiscalConfig(req: Request, res: Response) {
    try {
      const storeId = req.params.id as string;
      const store = await prisma.store.findFirst({ where: { id: storeId, control: { clientId: req.user?.clientId as string } } });
      if (!store) return fail(res, 'Loja não encontrada', 404);
  ...
  async updateDashboardConfig(req: Request, res: Response) {
    try {
      ...
      console.error('Erro ao atualizar config do dashboard:', error);
      return fail(res, 'Erro interno', 500);
    }
  }
}
